<script>
    ...
    import PWCToken from "./artifacts/contracts/PWCToken.sol/PWCToken.json";
    import PWCShop from "./artifacts/contracts/PWCShop.sol/PWCShop.json";
    ...
    const getPwCTokenBalance = async () => {
        if (typeof window.ethereum !== "undefined") {
            const contract = getContract(pwcTokenAddress, PWCToken.abi);
            try {
    ...
    const transferPWCToken = async () => {
        if (typeof window.ethereum !== "undefined") {
            ...
            try {
                ...
                const transaction = await contract.transfer('0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC', valueToTransfer)
    ...
    const buyTicket = async (ticket) => {
        console.log(ticket);
        if (typeof window.ethereum !== "undefined") {
            const contract = getContract(pwcShopAddress, PWCShop.abi);
    ...
    async function listenToEvents() {
        ...
        if (typeof window.ethereum !== 'undefined') {
            ...
            const provider = new ethers.providers.Web3Provider(window.ethereum);
            const contract = new ethers.Contract(pwcTokenAddress, PWCToken.abi, provider);
